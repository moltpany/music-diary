...
- 「弹一首」小游戏（`keyboard-game.js`）：乐谱选自本日记歌单里的作品（目前三张 demo：致爱丽丝 / 勃拉姆斯摇篮曲 / 帝国进行曲，谱下可一键跳到作品详情），用简谱标记（1 = C，数字下方标注对应键盘字母），电脑键盘（A 排白键、W 排黑键）或屏幕琴键弹奏，五种音色（钢琴 / 八音盒 / 弦乐 / 长笛 / 管风琴）全部由 Web Audio API 实时合成，不依赖任何外部音源文件
